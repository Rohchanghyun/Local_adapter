...
def visualize_attention_maps(attention, save_path, title, data=None, batch_idx=None):
    ...
    if data is not None and 'img' in data:
        ...
        # target이 있는 경우에만 keypoint 시각화
        if data.get('target') is not None and data.get('target_weight') is not None:
            plt.figure(figsize=(10, 10))
            plt.imshow(cv2.cvtColor(img, cv2.COLOR_BGR2RGB))
    ...
    if not distributed:
        ...
        for batch_idx, data in enumerate(data_loader):
            with torch.no_grad():
                ...
                # attention map 저장 로직 추가
                if batch_idx < 20:  # 처음 20개 배치만 처리
                    print(f"\nProcessing batch {batch_idx}")
                    
                    # MCT attention maps 처리
                    for att_name, attention_list in feature_extractor.attentions.items():
                        if 'transformer_layer' in att_name and len(attention_list) > batch_idx:
                            # 현재 배치의 attention map 가져오기
                            attention = attention_list[batch_idx]
                            
                            # 각 이미지에 대해 개별적으로 처리
                            for img_idx in range(min(2, attention.size(0))):  # 배치의 첫 두 이미지만 처리
